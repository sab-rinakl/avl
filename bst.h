...
protected:
    ...
    virtual void nodeSwap(Node<Key, Value>* n1, Node<Key, Value>* n2); // swaps given nodes
    int getHeight(Node<Key, Value>* n) const;        // returns height of node
    bool subtreeBalance(Node<Key, Value>* n) const;  // returns true if subtree with root n is balanced
    void subtreeClear(Node<Key, Value>* n);          // recursively clears subtree with root n
...
typename BinarySearchTree<Key, Value>::iterator& BinarySearchTree<Key, Value>::iterator::operator++() {
    if (current_->getRight() != nullptr)  // if current_ has a right child
    {
        current_ = current_->getRight();
        while (current_->getLeft() != nullptr)  // goes to leftmost node of right child
        {
    ...
    } else  // no right child
    {
        ...
        while (parent != nullptr && current_ == parent->getRight())  // goes left up ancestor chain
        {
            current_ = parent;
...
void BinarySearchTree<Key, Value>::insert(const std::pair<const Key, Value>& keyValuePair) {
    if (root_ == nullptr)  // if tree is empty
    {
        ...
        Node<Key, Value>* curr = root_;  // finds where new node will go
        Node<Key, Value>* prev = root_;  // tracks curr's parent
        while (curr != nullptr) {
            prev = curr;
            if (curr->getKey() == keyValuePair.first)  // if key is already in table
            {
                curr->setValue(keyValuePair.second);
                return;
            } else if (curr->getKey() > keyValuePair.first)  // key greater than curr's key
            {
                curr = curr->getLeft();
            } else  // key less than curr's key
            {
                curr = curr->getRight();
            }
        }
        if (prev->getKey() > keyValuePair.first)  // new node will be left child
        {
            ...
            prev->setRight(new Node<Key, Value>(
                    keyValuePair.first, keyValuePair.second, prev));  // new node will be right child
...
Node<Key, Value>* BinarySearchTree<Key, Value>::predecessor(Node<Key, Value>* current) {
    ...
    {
        current = current->getLeft();        // go to left subtree
        while (current->getRight() != NULL)  // find rightmost child
        {
        ...
        while (current->getParent() != NULL
               && current->getParent()->getRight() == NULL)  // find first right child pointer
        {
            current = current->getParent();  // walk up ancestor chain
        }
        if (current->getParent() == NULL && current->getRight() == NULL)  // no right child in tree
        {
            return NULL;
        }
        return current;
...
void BinarySearchTree<Key, Value>::clear() {
    while (!empty()) {
        subtreeClear(this->root_);
        this->root_ = nullptr;
    }
}

// recursively clears subtree with root n
template<typename Key, typename Value>
void BinarySearchTree<Key, Value>::subtreeClear(Node<Key, Value>* n) {
    if (n == nullptr) {
        return;
    }
    subtreeClear(n->getLeft());
    subtreeClear(n->getRight());
    n->setParent(nullptr);
    n->setLeft(nullptr);
    n->setRight(nullptr);
    delete n;
...
Node<Key, Value>* BinarySearchTree<Key, Value>::getSmallestNode() const {
    ...
    while (n->getLeft() != NULL)  // finds leftmost/smallest node
    {
        n = n->getLeft();
    }
    return n;
...
Node<Key, Value>* BinarySearchTree<Key, Value>::internalFind(const Key& key) const {
    ...
    while (temp != nullptr)  // finds node with given key
    {
        if (temp->getKey() == key) {
            return temp;
    ...
    return NULL;  // if node is not in tree
}
...
// returns true if subtree with root n is balanced
template<typename Key, typename Value>
bool BinarySearchTree<Key, Value>::subtreeBalance(Node<Key, Value>* n) const {
    // from lab 11
    if (n == nullptr) {
        return true;
    }
    ...
    bool leftBalance = subtreeBalance(n->getLeft());
    bool rightBalance = subtreeBalance(n->getRight());
